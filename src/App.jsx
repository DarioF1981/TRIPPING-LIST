...
function GestioneLista({ catMap, catNames, customItems, removedIds, expiry, onSave, onClose, onImport }) {
  ...
  // ── Export: pack all user data into a JSON file and share ──────────────────
  const handleExport = () => {
    try {
      const KEYS = [
      ...
      a.href     = url;
      a.download = `smartbit-backup-${new Date().toISOString().split("T")[0]}.json`;
...
export default function App() {
  ...
  const [excluded, setExcluded] = useState(() => {
    try { const p = JSON.parse(localStorage.getItem(K_PROGRESS)); return p?.excluded || []; } catch { return []; }
  });
  ...
  const confirmTripSetup = ({ selected, tag }) => {
    ...
    setTripTag(tag || "");
    try {
      localStorage.setItem(K_HIDDEN, JSON.stringify(newHidden));
      localStorage.setItem(K_TRIPTAG, tag || "");
    } catch {}
    setShowProfile(false);
  };

  // Reset: clear trip progress, show group selector with user's saved groups
  const doReset = () => {
    setCheckedIds([]);
    setQtyState({});
    setExcluded([]);
    ...
    return <GestioneLista
      ...
      onImport={() => {
        ...
        try {
          ...
          const p = JSON.parse(localStorage.getItem("cv_progress_v4"));
          setCheckedIds(p?.checkedIds || []);
          setQtyState(p?.qtyState || {});
          setExcluded(p?.excluded || []);
          setDeparture(localStorage.getItem("cv_departure_v4") || "");
          setDestination(localStorage.getItem("cv_dest_v4") || "");
          setHistory(JSON.parse(localStorage.getItem("cv_history_v4")) || []);
          // Reset hidden groups so all imported groups are visible in the trip selector
          setHiddenGroups([]);
          localStorage.setItem(K_HIDDEN, "[]");
          setTripTag("");
          localStorage.setItem(K_TRIPTAG, "");
    ...
    <div style={{ fontFamily:"'Segoe UI',system-ui,sans-serif", background:"#0F172A", minHeight:"100vh", color:"#F1F5F9", paddingBottom:40 }}>
      ...
      <div style={{ background:"#1E293B", padding:"calc(16px + env(safe-area-inset-top, 24px)) 20px 12px", position:"sticky", top:0, zIndex:10, borderBottom:"1px solid #334155" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
          ...
          {/* Trip tag badge */}
          {tripTag && tripTag !== "personale" && (() => {
            const TAGS = { mare:"🏖 Mare", lavoro:"💼 Lavoro", montagna:"🏔 Montagna" };
            const COLORS = { mare:"#38BDF8", lavoro:"#EF4444", montagna:"#22C55E" };
            const label = TAGS[tripTag] || tripTag;
            const color = COLORS[tripTag] || "#F97316";
            return (
              <div style={{
