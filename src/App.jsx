...
// Plugin nativo custom (vedi SaveFilePlugin.java): apre il selettore di
// cartelle/file di sistema Android per salvare davvero il backup dove vuole
// l'utente, invece del solo pannello "Condividi".
const SaveFile = registerPlugin("SaveFile");

// Bump this string every time a new build is shipped, so it's possible to confirm
// from a screenshot alone whether the phone is really running the latest code.
const BUILD_VERSION = "2026-09-21.2";
...
function slotBadgeInfo(tag, customTags) {
  if (!tag || tag === "personale") return null; // inactive / unnamed personale
  const custom = customTags && customTags.find(c => c.key === tag);
...
const K_KNOWNGROUPS = "cv_knowngroups_v4"; // every group ever created, kept even with 0 items — only Gestione's delete removes one
...
const K_BACKUPFOLDER = "cv_backupfolder_uri_v1"; // cartella (SAF tree URI) scelta per i backup automatici
const K_CUSTOMTAGS = "cv_customtags_v1"; // tipi di viaggio personalizzati salvati (oltre a Mare/Lavoro/Montagna)
const CUSTOM_TAG_COLORS = ["#EC4899","#8B5CF6","#06B6D4","#F59E0B","#84CC16","#F97316","#3B82F6","#EF4444"];
...
function TripSetupModal({ allCatMap, catNames, hiddenGroups, onConfirm, onClose,
  customTags, onAddCustomTag, onDeleteCustomTag, onRenameCustomTag }) {
  ...
  // Gestione dei viaggi personalizzati salvati (es. "🚲 Bici"): aggiungi, rinomina, elimina
  const [addingCustomTag, setAddingCustomTag] = useState(false);
  const [newCustomTagName, setNewCustomTagName] = useState("");
  const [editingCustomTag, setEditingCustomTag] = useState(null); // key in modifica
  const [renameValue, setRenameValue] = useState("");
  const [confirmDeleteTag, setConfirmDeleteTag] = useState(null); // key da eliminare
  ...
  const accent = tag === "personale" ? "#F97316"
    : (TAGS.find(t=>t.key===tag)?.color || customTags.find(c=>c.key===tag)?.color || "#3B82F6");
    ...
    <div style={{ position:"fixed", inset:0, background:"#0F172A", zIndex:300,
      ...
      <div style={{ flex:1, overflowY:"auto", padding:"16px" }}>
        ...
        <div style={{ marginBottom:16 }}>
          ...
          {/* I tuoi viaggi salvati: es. "🚲 Bici" — restano disponibili anche per i prossimi viaggi */}
          <div style={{ fontSize:11, color:"#64748B", margin:"14px 0 8px", fontWeight:600, textTransform:"uppercase", letterSpacing:"0.05em" }}>
            I tuoi viaggi salvati
          </div>
          <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
            {customTags.map(ct => {
              const isEditing = editingCustomTag === ct.key;
              const selectedThis = tag === ct.key;
              if (isEditing) {
                return (
                  <div key={ct.key} style={{ display:"flex", gap:6, alignItems:"center", flex:1, minWidth:160 }}>
                    <input autoFocus value={renameValue} onChange={e=>setRenameValue(e.target.value)}
                      onKeyDown={e=>{
                        if(e.key==="Enter" && renameValue.trim()){ onRenameCustomTag(ct.key, renameValue.trim()); setEditingCustomTag(null); }
                        if(e.key==="Escape") setEditingCustomTag(null);
                      }}
                      style={{ flex:1, background:"#1E293B", border:`1px solid ${ct.color}`, borderRadius:8,
                        color:"#F1F5F9", fontSize:13, padding:"8px 10px", outline:"none" }}/>
                    <button onClick={()=>{ if(renameValue.trim()){ onRenameCustomTag(ct.key, renameValue.trim()); setEditingCustomTag(null); } }}
                      style={{ background:ct.color, border:"none", borderRadius:8, color:"#fff", fontWeight:700, fontSize:13, padding:"8px 10px", cursor:"pointer" }}>✓</button>
                    <button onClick={()=>setEditingCustomTag(null)}
                      style={{ background:"#334155", border:"none", borderRadius:8, color:"#94A3B8", fontWeight:700, fontSize:13, padding:"8px 10px", cursor:"pointer" }}>✗</button>
                  </div>
                );
              }
              return (
                <div key={ct.key} style={{ position:"relative", flex:1, minWidth:88 }}>
                  <button onClick={()=>setTag(ct.key)} style={{
                    width:"100%", padding:"10px 4px", borderRadius:10,
                    border: selectedThis ? `2px solid ${ct.color}` : "2px solid #334155",
                    background: selectedThis ? `${ct.color}22` : "#1E293B",
                    color: selectedThis ? ct.color : "#64748B",
                    fontWeight:700, fontSize:12, cursor:"pointer"
                  }}>{ct.label}</button>
                  <div style={{ position:"absolute", top:-7, right:-6, display:"flex", gap:3 }}>
                    <span onClick={()=>{ setEditingCustomTag(ct.key); setRenameValue(ct.label); }} title="Rinomina" style={{
                      background:"#334155", borderRadius:"50%", width:19, height:19, display:"flex",
                      alignItems:"center", justifyContent:"center", fontSize:10, cursor:"pointer"
                    }}>✏️</span>
                    <span onClick={()=>setConfirmDeleteTag(ct.key)} title="Elimina" style={{
                      background:"#EF4444", borderRadius:"50%", width:19, height:19, display:"flex",
                      alignItems:"center", justifyContent:"center", fontSize:13, fontWeight:900, color:"#fff", cursor:"pointer"
                    }}>−</span>
                  </div>
                </div>
              );
            ...
            {!addingCustomTag ? (
              <button onClick={()=>{ setAddingCustomTag(true); setNewCustomTagName(""); }} style={{
                flex:1, minWidth:88, padding:"10px 4px", borderRadius:10,
                border:"2px dashed #334155", background:"#1E293B", color:"#3B82F6",
                fontWeight:700, fontSize:12, cursor:"pointer"
              }}>+ Nuovo viaggio</button>
            ) : (
              <div style={{ display:"flex", gap:6, alignItems:"center", flex:1, minWidth:180 }}>
                <input autoFocus value={newCustomTagName} onChange={e=>setNewCustomTagName(e.target.value)}
                  placeholder="es. 🚲 Bici"
                  onKeyDown={e=>{
                    if(e.key==="Enter" && newCustomTagName.trim()){ const key = onAddCustomTag(newCustomTagName.trim()); setTag(key); setAddingCustomTag(false); }
                    if(e.key==="Escape") setAddingCustomTag(false);
                  }}
                  style={{ flex:1, background:"#1E293B", border:"1px solid #3B82F6", borderRadius:8,
                    color:"#F1F5F9", fontSize:13, padding:"8px 10px", outline:"none" }}/>
                <button onClick={()=>{ if(newCustomTagName.trim()){ const key = onAddCustomTag(newCustomTagName.trim()); setTag(key); setAddingCustomTag(false); } }}
                  style={{ background:"#3B82F6", border:"none", borderRadius:8, color:"#fff", fontWeight:700, fontSize:13, padding:"8px 10px", cursor:"pointer" }}>✓</button>
                <button onClick={()=>setAddingCustomTag(false)}
                  style={{ background:"#334155", border:"none", borderRadius:8, color:"#94A3B8", fontWeight:700, fontSize:13, padding:"8px 10px", cursor:"pointer" }}>✗</button>
              </div>
            )}
          </div>
      ...
      {confirmDeleteTag && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.78)", zIndex:400,
                      display:"flex", alignItems:"center", justifyContent:"center", padding:24 }}>
          <div style={{ background:"#1E293B", borderRadius:16, padding:"24px 20px", width:"100%", maxWidth:320 }}>
            ...
            <div style={{ fontWeight:800, fontSize:16, color:"#F1F5F9", textAlign:"center", marginBottom:8 }}>
              Eliminare questo viaggio salvato?
            </div>
            <div style={{ fontSize:13, color:"#94A3B8", textAlign:"center", marginBottom:20, lineHeight:1.5 }}>
              «{customTags.find(c=>c.key===confirmDeleteTag)?.label}» non sarà più tra i tipi di viaggio proposti. I viaggi già fatti con questo nome non vengono toccati.
            </div>
            <div style={{ display:"flex", gap:10 }}>
              <button onClick={()=>setConfirmDeleteTag(null)} style={{ flex:1, background:"#334155", border:"none", borderRadius:12, padding:"13px 0", color:"#94A3B8", fontWeight:700, fontSize:15, cursor:"pointer" }}>Annulla</button>
              <button onClick={()=>{ onDeleteCustomTag(confirmDeleteTag); if(tag===confirmDeleteTag) setTag(""); setConfirmDeleteTag(null); }} style={{ flex:1, background:"#EF4444", border:"none", borderRadius:12, padding:"13px 0", color:"#fff", fontWeight:800, fontSize:15, cursor:"pointer" }}>Elimina</button>
            </div>
          </div>
        </div>
      )}
...
function AddItemModal({ categories, onAdd, onClose }) {
  ...
  // Difensivo: se per qualsiasi motivo la categoria selezionata non è (più)
  // tra quelle disponibili per il viaggio corrente, riallinea alla prima
  // categoria valida invece di lasciare selezionata una categoria "fantasma".
  useEffect(() => {
      ...
      <div onClick={e=>e.stopPropagation()}
        ...
        <div style={{ marginBottom:12 }}>
          ...
          <div style={{ fontSize:9, color:"#475569", marginTop:8 }}>
            debug build {BUILD_VERSION} · {categories.length} gruppi: {categories.join(" | ")}
          </div>
        </div>
        <div style={{ marginBottom:12 }}>
...
// ── Gestione Lista view ───────────────────────────────────────────────────────
function GestioneLista({ catMap, catNames, customItems, removedIds, expiry, onSave, onClose, onImport, onOpenAdd }) {
  ...
  // Cartella scelta una volta sola dall'utente per i backup automatici
  // (persistita via SAF: l'app mantiene il permesso di scriverci anche
  // dopo aver chiuso e riaperto l'app, senza richiedere il selettore ogni volta).
  const [backupFolderUri, setBackupFolderUri] = useState(() => {
    try { return localStorage.getItem(K_BACKUPFOLDER) || null; } catch { return null; }
  });

  const pad2 = (n) => String(n).padStart(2, "0");
  const backupTimestampName = () => {
    const d = new Date();
    return `ReadyGo_${d.getFullYear()}-${pad2(d.getMonth()+1)}-${pad2(d.getDate())}_${pad2(d.getHours())}-${pad2(d.getMinutes())}.json`;
  };

  // Apre il selettore di cartelle Android: l'utente sceglie o crea (es. "ReadyGo")
  // la cartella dove andranno tutti i backup futuri, senza richiederla ogni volta.
  const chooseBackupFolder = async () => {
    try {
      const { uri } = await SaveFile.pickFolder();
      setBackupFolderUri(uri);
      try { localStorage.setItem(K_BACKUPFOLDER, uri); } catch {}
      return uri;
    } catch (e) {
      if (e && typeof e.message === "string" && e.message.toLowerCase().includes("annullat")) return null;
      console.error("pickFolder error:", e);
      return null;
    }
  };

  // Costruisce il JSON di backup con tutti i dati salvati (entrambi gli slot A/B).
  const buildBackupPayload = () => {
    const KEYS = [
      ...
      "cv_deletedgroups_v4", "cv_currentslot_v4", "cv_knowngroups_v4",
    ...
    const filename = backupTimestampName();
    return { json, filename };
  };

  // ── Salva su dispositivo: la prima volta chiede di scegliere/creare una
  // cartella (es. "ReadyGo"); da quel momento ogni backup finisce lì in
  // automatico, con nome data e ora, senza più aprire alcun selettore.
  const handleSaveToDevice = async () => {
    try {
      ...
      if (!folderUri) {
        folderUri = await chooseBackupFolder();
        if (!folderUri) return; // annullato dall'utente
      }
      try {
        await SaveFile.saveToFolder({ treeUri: folderUri, filename, data: json, mimeType: "application/json" });
      } catch (folderErr) {
        // La cartella salvata potrebbe non essere più valida (spostata/permesso revocato):
        // richiedi di sceglierla di nuovo e riprova una sola volta.
        console.error("saveToFolder fallito, richiedo nuova cartella:", folderErr);
        const newFolderUri = await chooseBackupFolder();
        if (!newFolderUri) throw folderErr;
        await SaveFile.saveToFolder({ treeUri: newFolderUri, filename, data: json, mimeType: "application/json" });
      }
  ...
  const handleExport = async () => {
    try {
      const { json, filename } = buildBackupPayload();

      // Metodo principale: plugin nativi Capacitor (Filesystem + Share).
      // A differenza della Web Share API, funzionano davvero dentro la WebView
      // Android di Capacitor e aprono il vero pannello "Condividi con..." di Android.
      try {
        ...
        await Share.share({
          title: "Backup Ready!Go",
          text: "Backup della lista Ready!Go",
          url: uri,
        ...
        if (nativeErr && (nativeErr.message === "Share canceled" || nativeErr.message === "Share canceled.")) {
          // Utente ha chiuso il pannello di condivisione: non è un errore.
          return;
        }
        // Plugin non disponibile o fallito: prova i metodi di riserva sotto.
        console.error("Native export error:", nativeErr);
      }

      const blob = new Blob([json], { type: "application/json" });

      // Metodo di riserva 1: Web Share API (funziona in alcuni browser/WebView).
      ...
      // Metodo di riserva 2: download diretto via blob (funziona nei browser normali,
  ...
  return (
    <div style={{ fontFamily:"'Segoe UI',system-ui,sans-serif", background:"#0F172A", minHeight:"100vh", color:"#F1F5F9", paddingBottom:200 }}>
      {/* Header — fissato in alto: resta visibile anche scorrendo la lista */}
      <div style={{ background:"#1E293B", position:"sticky", top:0, zIndex:10, borderBottom:"1px solid #334155" }}>
        <div style={{ padding:"calc(16px + env(safe-area-inset-top, 24px)) 20px 12px", display:"flex", alignItems:"center", gap:12 }}>
          <button onClick={onClose} style={{ background:"none", border:"none", color:"#94A3B8", fontSize:22, cursor:"pointer", padding:0 }}>←</button>
          <div style={{ flex:1 }}>
            ...
            <div style={{ fontSize:9, color:"#334155", marginTop:1 }}>build {BUILD_VERSION}</div>
          </div>
        </div>
        {/* Barra azioni fissa — riga 1: Aggiungi elemento / Nuovo gruppo */}
        <div style={{ display:"flex", gap:7, padding:"0 16px 8px" }}>
          <button onClick={onOpenAdd} style={{ flex:1, minWidth:70, background:"#0F172A", border:"1px solid #3B82F6", color:"#3B82F6", borderRadius:8, padding:"9px 4px", fontSize:12, fontWeight:700, cursor:"pointer" }}>➕ Aggiungi elemento</button>
          <button onClick={()=>setShowNewGroup(true)} style={{ flex:1, minWidth:70, background:"#0F172A", border:"1px solid #F59E0B", color:"#F59E0B", borderRadius:8, padding:"9px 4px", fontSize:12, fontWeight:700, cursor:"pointer" }}>＋ Nuovo gruppo</button>
        </div>
        {/* Barra azioni fissa — riga 2: Salva / Condividi / Ripristina */}
        <div style={{ display:"flex", gap:7, padding:"0 16px 12px", flexWrap:"wrap" }}>
          <button onClick={handleSaveToDevice} style={{ flex:1, minWidth:70, background:"#0F172A", border:"1px solid #10B981", color:"#10B981", borderRadius:8, padding:"9px 4px", fontSize:12, fontWeight:700, cursor:"pointer" }}>💾 Salva</button>
          <button onClick={handleExport} style={{ flex:1, minWidth:70, background:"#0F172A", border:"1px solid #10B981", color:"#10B981", borderRadius:8, padding:"9px 4px", fontSize:12, fontWeight:700, cursor:"pointer" }}>📤 Condividi</button>
          <label style={{ flex:1, minWidth:70, background:"#0F172A", border:"1px solid #10B981", color:"#10B981", borderRadius:8, padding:"9px 4px", fontSize:12, fontWeight:700, cursor:"pointer", textAlign:"center" }}>
            📥 Ripristina
        ...
        <div style={{ padding:"0 16px 12px", fontSize:11, color:"#64748B" }}>
          {backupFolderUri
            ? <>📁 Cartella backup impostata — <span onClick={chooseBackupFolder} style={{ color:"#3B82F6", cursor:"pointer", fontWeight:600 }}>cambia</span></>
            : <>📁 Nessuna cartella backup scelta — verrà chiesta al primo tocco su <b>💾 Salva</b></>
          }
        </div>
        {exportFeedback === "ok" && (
          <div style={{ padding:"0 16px 10px", fontSize:12, color:"#3B82F6", fontWeight:700, textAlign:"center" }}>
            ✅ Backup pronto
          </div>
        )}
        {exportFeedback === "errore" && (
          <div style={{ padding:"0 16px 10px", fontSize:12, color:"#EF4444", fontWeight:700, textAlign:"center" }}>
        ...
        {importOk && (
          <div style={{ padding:"0 16px 10px", fontSize:12, color:"#10B981", fontWeight:700, textAlign:"center" }}>
            ✅ Dati ripristinati! L'app si aggiorna...
          </div>
        )}
        {showNewGroup && (
          <div style={{ margin:"0 16px 14px", background:"#0F172A", borderRadius:10, padding:"14px 16px", border:"1px solid #F59E0B" }}>
            ...
            <div style={{ display:"flex", gap:8 }}>
              ...
              <button onClick={()=>{ if(newGroupName.trim()) setShowNewGroup(false); }} style={{
                flex:2, background: newGroupName.trim()?"#F59E0B":"#283548",
                border:"none", borderRadius:8, padding:"10px 0",
                color: newGroupName.trim()?"#0F172A":"#475569", fontWeight:700, fontSize:14, cursor:"pointer"
              }}>✓ Conferma nome</button>
            </div>
            {newGroupName.trim() && (
              <div style={{ marginTop:8, fontSize:11, color:"#10B981" }}>
                ✓ Il gruppo «{newGroupName.trim()}» verrà creato al salvataggio. Usa ➕ Aggiungi elemento qui sopra per aggiungere elementi.
              </div>
            )}
          </div>
        )}
...
export default function App() {
  ...
  // Every group ever created (default + custom), so a group never silently disappears
  // just because it currently has zero items — only an explicit delete in Gestione removes it.
  const [knownGroups, setKnownGroups] = useState(() => {
    try {
      const stored = JSON.parse(localStorage.getItem(K_KNOWNGROUPS)) || [];
      return [...new Set([...Object.keys(DEFAULT_DATA), ...stored])];
    } catch { return Object.keys(DEFAULT_DATA); }
  });
  const [tripTag, setTripTag] = useState(() => {
    try { return localStorage.getItem(sk(K_TRIPTAG, currentSlot)) || ""; } catch { return ""; }
  });
  // Tipi di viaggio personalizzati e salvati dall'utente (oltre a Mare/Lavoro/Montagna),
  // così restano disponibili come scelta rapida anche nei prossimi viaggi.
  const [customTags, setCustomTags] = useState(() => {
    try { return JSON.parse(localStorage.getItem(K_CUSTOMTAGS)) || []; } catch { return []; }
  });
  useEffect(() => { try { localStorage.setItem(K_CUSTOMTAGS, JSON.stringify(customTags)); } catch {} }, [customTags]);

  const addCustomTag = (label) => {
    const base = label.toLowerCase().normalize("NFD").replace(/[̀-ͯ]/g,"")
      .replace(/[^a-z0-9]+/g,"-").replace(/(^-|-$)/g,"") || "viaggio";
    let key = base, i = 1;
    while (customTags.some(c => c.key === key)) { key = `${base}-${i++}`; }
    const color = CUSTOM_TAG_COLORS[customTags.length % CUSTOM_TAG_COLORS.length];
    setCustomTags(prev => [...prev, { key, label, color }]);
    return key;
  };
  const deleteCustomTag = (key) => setCustomTags(prev => prev.filter(c => c.key !== key));
  const renameCustomTag = (key, newLabel) =>
    setCustomTags(prev => prev.map(c => c.key === key ? { ...c, label: newLabel } : c));
  ...
  useEffect(() => { try { localStorage.setItem(K_KNOWNGROUPS, JSON.stringify(knownGroups)); } catch {} }, [knownGroups]);

  // Build category map — every known group gets an entry (even empty), so it never
  // silently disappears just because it currently has zero items. Only an explicit
  // delete in Gestione (deletedGroupKeys) removes a group from here.
  const buildCatMap = () => {
    const map = {};
    for (const cat of knownGroups) {
  ...
  const addItem = ({ cat, nome, qty }) => {
    ...
    setKnownGroups(p => p.includes(cat) ? p : [...p, cat]);
  };
  ...
  const handleGestioneSave = ({ catNames: newNames, removedIds: newRemoved, expiry: newExpiry, qtys, newGroup, deletedGroups: delGroups }) => {
    ...
    if (newGroup) {
      ...
      setKnownGroups(prev => prev.includes(newGroup) ? prev : [...prev, newGroup]);
    }
    ...
    if (delGroups && delGroups.length > 0) {
      ...
      // A deleted group is no longer "known" either
      setKnownGroups(prev => prev.filter(k => !delGroups.includes(k)));
    }
  ...
  if (view === "gestione") {
    ...
    <GestioneLista
      ...
      onImport={() => {
        ...
        try {
          const importedCustom = JSON.parse(localStorage.getItem("cv_custom_v4")) || [];
          setCustomItems(importedCustom);
          ...
          // Rebuild knownGroups from: stored value (if the backup had it) + every
          // category actually used by imported items + defaults — so a group is
          // never lost even when restoring an older backup made before this fix.
          const storedKnown = (() => { try { return JSON.parse(localStorage.getItem(K_KNOWNGROUPS)) || []; } catch { return []; } })();
          const fromItems = [...new Set(importedCustom.map(ci => ci.cat))];
          const rebuiltKnown = [...new Set([...Object.keys(DEFAULT_DATA), ...storedKnown, ...fromItems])];
          setKnownGroups(rebuiltKnown);
          try { localStorage.setItem(K_KNOWNGROUPS, JSON.stringify(rebuiltKnown)); } catch {}
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
          ...
          <div style={{ display:"flex", gap:6 }}>
            {["a","b"].map(slot => {
              ...
              const info = slotBadgeInfo(tag, customTags);
              if (!info) {
