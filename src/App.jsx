...
// ── Default data — app starts empty, all groups are user-created (in Gestione) ──
const DEFAULT_DATA = {};

const DEFAULT_CAT_COLORS = {};
// Palette assigned automatically to user-created groups, in creation order
const AUTO_COLORS = ["#3B82F6", "#10B981", "#F59E0B", "#EF4444", "#8B5CF6", "#EC4899", "#14B8A6", "#F97316"];
...
const K_HIDDEN     = "cv_hidden_v4";    // group keys hidden from the current trip (not deleted)
const K_TRIPTAG    = "cv_triptag_v4";   // optional label: "lavoro" | "mare" | "montagna" | ""
const K_EXPIRYGROUPS = "cv_expirygroups_v4"; // group keys that track expiry dates
const K_EMPTYGROUPS  = "cv_emptygroups_v4";  // groups created with no items yet — kept visible


// Whether a group tracks expiry dates is now an explicit choice made when the
// group is created (see expiryGroups state in App / GestioneLista), not guessed from its name.
...
function HistoryView({ history, onDelete, onClose }) {
  ...
  return (
    <div style={{ fontFamily:"'Segoe UI',system-ui,sans-serif", background:"#0F172A", minHeight:"100vh", color:"#F1F5F9", paddingBottom:"calc(40px + env(safe-area-inset-bottom, 16px))" }}>
...
// ── Add Item Modal ────────────────────────────────────────────────────────────
function AddItemModal({ categories, catNames, onAdd, onClose }) {
        ...
        <div style={{ marginBottom:12 }}>
          ...
          {!useNew
            ? <select value={cat} onChange={e=>setCat(e.target.value)} style={{ width:"100%", background:"#0F172A", border:"1px solid #334155", borderRadius:10, color:"#F1F5F9", fontSize:14, padding:"10px 14px", boxSizing:"border-box", outline:"none" }}>
                {categories.map(c=><option key={c} value={c}>{(catNames && catNames[c]) || c}</option>)}
              </select>
...
// ── Gestione Lista view ───────────────────────────────────────────────────────
function GestioneLista({ catMap, catNames, customItems, removedIds, expiry, hiddenGroups, onUnhide, expiryGroups, tripTag, onTripTagChange, onSave, onClose }) {
  ...
  const [localItemNames, setLocalItemNames] = useState({}); // { itemId: newName } — renamed items, applied on Salva
  const [editingItemId, setEditingItemId]   = useState(null); // item id currently being renamed
  const [deletedGroups, setDeletedGroups]= useState([]); // group keys to delete entirely
  const [editingCat,    setEditingCat]   = useState(null);
  const [openCats,      setOpenCats]     = useState({});
  const [showNewGroup,  setShowNewGroup] = useState(false);
  const [newGroupName,  setNewGroupName] = useState("");
  const [newGroupHasExpiry, setNewGroupHasExpiry] = useState(false);
  const [localExpiryGroups, setLocalExpiryGroups] = useState([...(expiryGroups||[])]);
  const [editingTripTag, setEditingTripTag] = useState(false);
  const [tripTagDraft, setTripTagDraft] = useState(tripTag || "");
  const PRESET_TAGS = { lavoro: "💼 Lavoro", mare: "🏖 Mare", montagna: "🏔 Montagna" };
  const [localNewItems, setLocalNewItems]= useState([]); // items added in this session, not yet saved
  const [showAddItem,   setShowAddItem]  = useState(false);

  const allCategories = Object.keys(catMap).filter(k => !deletedGroups.includes(k));
  // Categories available to pick from when adding a new item — includes any
  // brand-new group the user just named but hasn't saved yet.
  const categoriesForAdd = newGroupName.trim()
    ? [...allCategories, newGroupName.trim()]
    : allCategories;
  ...
  // Add a new draft item — stays local until "Salva Lista" is pressed
  const addDraftItem = ({ cat, nome, qty }) => {
    setLocalNewItems(p => [...p, { id: `c${Date.now()}${Math.floor(Math.random()*1000)}`, cat, nome, qty }]);
  };

  const handleSave = () => {
    onSave({
      catNames: localNames,
      removedIds: localRemoved,
      expiry: localExpiry,
      qtys: localQtys,
      itemNames: localItemNames,
      newGroup: newGroupName.trim()||null,
      deletedGroups,
      newItems: localNewItems,
      expiryGroups: localExpiryGroups,
    });
  };

  return (
    <div style={{ fontFamily:"'Segoe UI',system-ui,sans-serif", background:"#0F172A", minHeight:"100vh", color:"#F1F5F9", paddingBottom:"calc(100px + env(safe-area-inset-bottom, 16px))" }}>
      {/* Header */}
      <div style={{ background:"#1E293B", padding:"calc(16px + env(safe-area-inset-top, 24px)) 20px 16px", display:"flex", alignItems:"center", gap:12,
                    position:"sticky", top:0, zIndex:10, borderBottom:"1px solid #334155" }}>
        <button onClick={onClose} style={{ background:"none", border:"none", color:"#94A3B8", fontSize:22, cursor:"pointer", padding:0 }}>←</button>
      ...
      <div style={{ padding:"12px 16px" }}>
        {/* Trip tag — editable any time */}
        <div style={{ marginBottom:14 }}>
          <div style={{ fontSize:11, color:"#64748B", marginBottom:8, fontWeight:600, textTransform:"uppercase", letterSpacing:"0.05em" }}>
            Tag viaggio corrente
          </div>
          {editingTripTag ? (
            <div style={{ background:"#1E293B", borderRadius:10, padding:"12px 14px", border:"1px solid #3B82F6" }}>
              <input
                value={tripTagDraft}
                onChange={e=>setTripTagDraft(e.target.value)}
                placeholder="es. Cugino Marco, Fiera Milano..."
                autoFocus
                style={{ width:"100%", background:"#0F172A", border:"1px solid #334155", borderRadius:8,
                  color:"#F1F5F9", fontSize:14, padding:"9px 12px", boxSizing:"border-box", outline:"none", marginBottom:10 }}
              />
              <div style={{ display:"flex", gap:8 }}>
                <button onClick={()=>{ setEditingTripTag(false); setTripTagDraft(tripTag||""); }} style={{
                  flex:1, background:"#334155", border:"none", borderRadius:8,
                  padding:"9px 0", color:"#94A3B8", fontWeight:700, fontSize:13, cursor:"pointer"
                }}>Annulla</button>
                <button onClick={()=>{ onTripTagChange(tripTagDraft.trim()); setEditingTripTag(false); }} style={{
                  flex:2, background:"#10B981", border:"none", borderRadius:8, padding:"9px 0",
            ...
            <div style={{ display:"flex", alignItems:"center", gap:8, background:"#1E293B",
              ...
              <span style={{ flex:1, fontSize:14, fontWeight:700, color: tripTag ? "#3B82F6" : "#475569" }}>
                {tripTag ? (PRESET_TAGS[tripTag] || `✏️ ${tripTag}`) : "Nessun tag impostato"}
              </span>
              <button onClick={()=>{ setTripTagDraft(tripTag||""); setEditingTripTag(true); }} style={{
                background:"none", border:"none", color:"#3B82F6", fontSize:14, cursor:"pointer", padding:"2px 4px"
              }} title="Modifica tag">✏️</button>
              {tripTag && (
                <button onClick={()=>onTripTagChange("")} style={{
                  background:"none", border:"none", color:"#EF4444", fontSize:16, cursor:"pointer", padding:"2px 4px"
                }} title="Rimuovi tag">✕</button>
              )}
            </div>
          )}
        </div>
        ...
        <div style={{ background:"#1E293B", border:"1px solid #334155", borderRadius:10, padding:"10px 14px", marginBottom:14 }}>
          <div style={{ fontSize:12, color:"#94A3B8", lineHeight:1.6 }}>
            ...
            ✏️ Tocca la <b style={{color:"#F1F5F9"}}>matita</b> accanto alla riga per rinominarla &nbsp;·&nbsp;
            🔢 Modifica il <b style={{color:"#F1F5F9"}}>numero</b> per cambiare la quantità di default &nbsp;·&nbsp;
            📅 Attiva/disattiva le <b style={{color:"#F1F5F9"}}>date di scadenza</b> per il gruppo
          </div>
        </div>
        ...
        {allCategories.map(catKey => {
          ...
          const isHidden = hiddenGroups && hiddenGroups.includes(catKey);

          return (
            <div key={catKey} style={{ marginBottom:10, opacity: isHidden ? 0.55 : 1 }}>
              {/* Category header */}
                ...
                {editingCat === catKey
                  ...
                  : <>
                      <button onClick={()=>setEditingCat(catKey)} style={{
                        ...
                        {displayName}
                        {isHidden && <span style={{ fontSize:10, color:"#64748B", marginLeft:8 }}>(escluso da questo viaggio)</span>}
                      ...
                      <button
                        onClick={()=>setLocalExpiryGroups(p => p.includes(catKey) ? p.filter(x=>x!==catKey) : [...p, catKey])}
                        title="Gestisce date di scadenza"
                        style={{
                          background:"none", border:"none", fontSize:15, cursor:"pointer",
                          padding:"8px 6px", lineHeight:1,
                          opacity: localExpiryGroups.includes(catKey) ? 1 : 0.3
                        }}>📅</button>
              ...
              {isOpen && (
                <div style={{ background:"#1E293B", borderRadius:"0 0 10px 10px", overflow:"hidden" }}>
                  {items.map((item, idx) => {
                    ...
                    const isMed = expiryGroups.includes(catKey);
                    ...
                    const displayItemName = localItemNames[item.id] !== undefined ? localItemNames[item.id] : item.nome;
                    ...
                    return (
                      <div key={item.id} style={{
                        ...
                        <div style={{ display:"flex", alignItems:"center", gap:10, padding:"10px 14px" }}>
                          ...
                          {/* Name — tap toggles remove · ✏️ opens rename */}
                          {isRenaming ? (
                            <CatNameEditor value={displayItemName}
                              onSave={v=>{ setLocalItemNames(p=>({...p,[item.id]:v})); setEditingItemId(null); }}
            ...
            <div style={{ background:"#1E293B", borderRadius:"0 0 10px 10px", overflow:"hidden" }}>
              {localNewItems.map((item, idx) => (
                <div key={item.id} style={{
                  ...
                  <span style={{ fontSize:13, flex:1, color:"#CBD5E1" }}>{item.nome}</span>
                  <span style={{ fontSize:11, color:"#64748B" }}>{catNames[item.cat]||item.cat}</span>
                  <span style={{ fontSize:13, fontWeight:700, color:"#10B981" }}>×{item.qty}</span>
                  <button onClick={()=>setLocalNewItems(p=>p.filter(x=>x.id!==item.id))} style={{
        ...
        <div style={{ marginTop:6, marginBottom:10 }}>
          {!showNewGroup ? (
            ...
            <div style={{ background:"#1E293B", borderRadius:10, padding:"14px 16px", border:"1px solid #3B82F6" }}>
              ...
              />
              <button onClick={()=>setNewGroupHasExpiry(p=>!p)} style={{
                ...
                <div style={{
                  ...
                  background: newGroupHasExpiry ? "#F59E0B" : "transparent",
                  border:`2px solid ${newGroupHasExpiry ? "#F59E0B" : "#475569"}`,
                  display:"flex", alignItems:"center", justifyContent:"center"
                }}>
                  {newGroupHasExpiry && <span style={{ color:"#fff", fontSize:12, fontWeight:900 }}>✓</span>}
                </div>
                <span style={{ fontSize:13, color:"#CBD5E1", textAlign:"left" }}>
                  📅 Gestisce date di scadenza (es. medicine)
                </span>
              </button>
              <div style={{ display:"flex", gap:8 }}>
                <button onClick={()=>{ setShowNewGroup(false); setNewGroupName(""); setNewGroupHasExpiry(false); }} style={{
                  flex:1, background:"#334155", border:"none", borderRadius:8,
                ...
                <button onClick={()=>{
                  if(newGroupName.trim()) {
                    ...
                    if (newGroupHasExpiry) setLocalExpiryGroups(p => [...p, newGroupName.trim()]);
                  }
                }} style={{
              ...
              {newGroupName.trim() && (
                <div style={{ marginTop:8, fontSize:11, color:"#10B981" }}>
                  ✓ Il gruppo «{newGroupName.trim()}» verrà creato al salvataggio. Usa ➕ Aggiungi elemento per aggiungerci dentro.
                </div>
              )}
            </div>
          )}
        ...
        {/* ── Gruppi esclusi dal viaggio corrente — riattivabili ── */}
        {hiddenGroups && hiddenGroups.length > 0 && (
          <div style={{ background:"#1E293B", borderRadius:10, padding:"12px 14px", marginBottom:10, border:"1px solid #334155" }}>
            <div style={{ fontSize:12, color:"#64748B", marginBottom:8, fontWeight:600 }}>
              👁️‍🗨️ Esclusi da questo viaggio ({hiddenGroups.length})
            </div>
            {hiddenGroups.map(cat => (
              <div key={cat} style={{ display:"flex", alignItems:"center", justifyContent:"space-between",
                                       padding:"6px 0", borderTop:"1px solid #0F172A" }}>
                <span style={{ fontSize:13, color:"#94A3B8" }}>{catNames[cat]||cat}</span>
                <button onClick={()=>onUnhide(cat)} style={{
        ...
        <button onClick={()=>setShowAddItem(true)} style={{
          width:"100%", background:"#1E293B", border:"2px dashed #60A5FA",
          borderRadius:10, padding:"13px 0", color:"#60A5FA",
          fontWeight:700, fontSize:14, cursor:"pointer"
        }}>➕ Aggiungi elemento</button>
      </div>

      {/* Save button fixed at bottom */}
      <div style={{ position:"fixed", bottom:0, left:0, right:0, padding:"14px 16px calc(28px + env(safe-area-inset-bottom, 12px))", background:"#0F172A", borderTop:"1px solid #1E293B" }}>
        <div style={{ display:"flex", gap:10 }}>
...
// ── Trip Setup Modal (shown on Reset) ─────────────────────────────────────────
function TripSetupModal({ fullCatMap, catNames, hiddenGroups, tripTag, onConfirm, onClose }) {
  const allCats = Object.keys(fullCatMap);
  const [selected, setSelected] = useState(
    () => allCats.filter(c => !hiddenGroups.includes(c))
  );
  const isPresetTag = (t) => ["lavoro","mare","montagna"].includes(t);
  const startsCustom = !!tripTag && !isPresetTag(tripTag);
  const [tag, setTag] = useState(startsCustom ? "" : (tripTag || ""));
  // customTag: the confirmed custom tag text, or "" if none set
  const [customTag, setCustomTag] = useState(startsCustom ? tripTag : "");
  // editingCustomTag: true while the dedicated "type your tag" step is shown
  const [editingCustomTag, setEditingCustomTag] = useState(false);
  const [customTagDraft, setCustomTagDraft] = useState(startsCustom ? tripTag : "");
  ...
  const TAGS = [
    ...
    { key: "mare",     label: "🏖 Mare" },
    { key: "montagna", label: "🏔 Montagna" },
  ];

  const finalTag = customTag || tag;

  const confirmCustomTag = () => {
    setCustomTag(customTagDraft.trim());
    setTag(""); // custom tag and preset tags are mutually exclusive
    setEditingCustomTag(false);
  };

  // ── Dedicated step: typing the custom tag ──────────────────────────────────
  if (editingCustomTag) {
    return (
      <div style={{ position:"fixed", inset:0, background:"#0F172A", zIndex:300,
        ...
        <div style={{ background:"#1E293B", padding:"20px 20px 16px", borderBottom:"1px solid #334155" }}>
          <div style={{ fontWeight:800, fontSize:20, color:"#F1F5F9", marginBottom:4 }}>✏️ Tag personalizzato</div>
          <div style={{ fontSize:13, color:"#64748B" }}>Scrivi il nome del tag per questo viaggio</div>
        </div>
        <div style={{ flex:1, padding:"16px" }}>
          <input
            ...
            placeholder="es. Cugino Marco, Fiera Milano..."
            autoFocus
            style={{ width:"100%", background:"#1E293B", border:"1px solid #3B82F6", borderRadius:10,
              color:"#F1F5F9", fontSize:15, padding:"12px 14px", boxSizing:"border-box", outline:"none" }}
          />
        </div>
        <div style={{ padding:"14px 16px calc(28px + env(safe-area-inset-bottom, 12px))", background:"#0F172A", borderTop:"1px solid #1E293B" }}>
          <div style={{ display:"flex", gap:10 }}>
            <button onClick={()=>setEditingCustomTag(false)} style={{
              flex:1, background:"#334155", border:"none", borderRadius:12,
              padding:"14px 0", color:"#94A3B8", fontWeight:700, fontSize:15, cursor:"pointer"
            }}>Annulla</button>
            <button onClick={confirmCustomTag} disabled={!customTagDraft.trim()} style={{
              flex:2, background: customTagDraft.trim() ? "#10B981" : "#283548",
              border:"none", borderRadius:12, padding:"14px 0",
              color: customTagDraft.trim() ? "#fff" : "#475569", fontWeight:800, fontSize:15, cursor:"pointer"
            }}>✓ Conferma</button>
          </div>
        </div>
      </div>
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"#0F172A", zIndex:300,
      ...
      <div style={{ background:"#1E293B", padding:"20px 20px 16px", borderBottom:"1px solid #334155" }}>
        ...
        <div style={{ fontSize:13, color:"#64748B" }}>Quali liste vuoi usare per questo viaggio?</div>
      </div>

      <div style={{ flex:1, overflowY:"auto", padding:"16px" }}>
        {/* Optional tag — moved right under the title banner */}
        <div style={{ marginBottom:20 }}>
          <div style={{ fontSize:12, color:"#64748B", marginBottom:10, fontWeight:600, textTransform:"uppercase", letterSpacing:"0.05em" }}>
            Tag viaggio (opzionale)
          </div>

          {customTag ? (
            // Confirmed custom tag — shown with edit/remove controls
            <div style={{ display:"flex", alignItems:"center", gap:8, background:"#1E293B",
                          border:"2px solid #3B82F6", borderRadius:10, padding:"10px 14px" }}>
              <span style={{ flex:1, fontSize:14, fontWeight:700, color:"#3B82F6" }}>✏️ {customTag}</span>
              <button onClick={()=>{ setCustomTagDraft(customTag); setEditingCustomTag(true); }} style={{
                background:"none", border:"none", color:"#3B82F6", fontSize:14, cursor:"pointer", padding:"2px 4px"
              }} title="Modifica">✏️</button>
              <button onClick={()=>{ setCustomTag(""); setCustomTagDraft(""); }} style={{
                background:"none", border:"none", color:"#EF4444", fontSize:16, cursor:"pointer", padding:"2px 4px"
            ...
            <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
              {TAGS.map(t => (
                <button key={t.key} onClick={()=>setTag(p=>p===t.key?"":t.key)} style={{
                  flex:1, minWidth:90, padding:"10px 0", borderRadius:10,
                  border: tag===t.key ? "2px solid #3B82F6" : "2px solid #334155",
                  background: tag===t.key ? "rgba(59,130,246,.15)" : "#1E293B",
                  color: tag===t.key ? "#3B82F6" : "#94A3B8",
                  fontWeight:700, fontSize:13, cursor:"pointer"
                }}>{t.label}</button>
              ))}
              <button onClick={()=>{ setCustomTagDraft(""); setEditingCustomTag(true); }} style={{
                flex:1, minWidth:90, padding:"10px 0", borderRadius:10,
        ...
        {allCats.length === 0 && (
          <div style={{ textAlign:"center", color:"#475569", fontSize:13, marginTop:40 }}>
            Nessuna lista salvata ancora.<br/>Crea i tuoi gruppi in ⚙️ Gestione.
          </div>
        )}
        {allCats.map(cat => {
          const included = selected.includes(cat);
          const color = DEFAULT_CAT_COLORS[cat] || "#64748B";
          const itemCount = (fullCatMap[cat]||[]).length;
          return (
            <button key={cat} onClick={()=>toggle(cat)} style={{
              width:"100%", background: included?"#1E293B":"#0F172A",
              border:`2px solid ${included ? color : "#334155"}`,
              ...
              <div style={{
                ...
                border:`2px solid ${included ? color : "#475569"}`,
                display:"flex", alignItems:"center", justifyContent:"center"
              }}>
                {included && <span style={{ color:"#fff", fontSize:14, fontWeight:900 }}>✓</span>}
              </div>
              <div style={{ flex:1 }}>
                <div style={{ fontWeight:700, fontSize:14, color: included?"#F1F5F9":"#64748B" }}>
                  {catNames[cat] || cat}
                </div>
      ...
      <div style={{ padding:"14px 16px calc(28px + env(safe-area-inset-bottom, 12px))", background:"#0F172A", borderTop:"1px solid #1E293B" }}>
        <div style={{ display:"flex", gap:10 }}>
          ...
          <button onClick={()=>onConfirm({ selected, tag: finalTag })} style={{
            flex:2, background:"#10B981", border:"none", borderRadius:12, padding:"14px 0",
...
export default function App() {
  ...
  const [removedIds, setRemovedIds] = useState(() => {
    try { return JSON.parse(localStorage.getItem(K_REMOVED)) || []; } catch { return []; }
  });

  // Category display names override
  const [catNames, setCatNames] = useState(() => {
    try { return JSON.parse(localStorage.getItem(K_CATNAMES)) || {}; } catch { return {}; }
  });
  ...
  const [confirmHideGroup, setConfirmHideGroup] = useState(null); // cat key pending hide confirmation
  ...
  // Groups hidden from the current trip (not deleted — chosen via the Reset/new-trip selector)
  const [hiddenGroups, setHiddenGroups] = useState(() => {
    try { return JSON.parse(localStorage.getItem(K_HIDDEN)) || []; } catch { return []; }
  });
  // Optional trip tag — label only, doesn't affect groups
  const [tripTag, setTripTag] = useState(() => {
    try { return localStorage.getItem(K_TRIPTAG) || ""; } catch { return ""; }
  });
  // Shows the "new trip" group-picker (opened by Reset)
  const [showTripSetup, setShowTripSetup] = useState(false);
  // Group keys that track expiry dates — chosen explicitly when each group is created
  const [expiryGroups, setExpiryGroups] = useState(() => {
    try { return JSON.parse(localStorage.getItem(K_EXPIRYGROUPS)) || []; } catch { return []; }
  });
  // Groups created via Gestione that have no items yet — kept visible until items are added
  const [emptyGroups, setEmptyGroups] = useState(() => {
    try { return JSON.parse(localStorage.getItem(K_EMPTYGROUPS)) || []; } catch { return []; }
  });
  ...
  useEffect(() => { try { localStorage.setItem(K_HIDDEN, JSON.stringify(hiddenGroups)); } catch {} }, [hiddenGroups]);
  useEffect(() => { try { localStorage.setItem(K_TRIPTAG, tripTag); } catch {} }, [tripTag]);
  useEffect(() => { try { localStorage.setItem(K_EXPIRYGROUPS, JSON.stringify(expiryGroups)); } catch {} }, [expiryGroups]);
  useEffect(() => { try { localStorage.setItem(K_EMPTYGROUPS, JSON.stringify(emptyGroups)); } catch {} }, [emptyGroups]);
  ...
  const buildCatMap = () => {
    ...
    // Keep groups that were created but have no items yet, so they stay visible
    for (const cat of emptyGroups) {
      if (!map[cat]) map[cat] = [];
    }
    return map;
  };
  const fullCatMap = buildCatMap();           // ALL saved groups — used by Gestione
  const catMap = Object.fromEntries(
    Object.entries(fullCatMap).filter(([cat]) => !hiddenGroups.includes(cat))
  );                                            // visible groups — used by the main list
  const allCategories = Object.keys(catMap);
  const allSavedCategories = Object.keys(fullCatMap); // for Gestione (includes hidden)
  // Each saved group gets a stable color based on its position in the full saved list,
  // so colors don't shift around when a group is hidden/restored.
  const getCatColor = (key) => {
    const idx = allSavedCategories.indexOf(key);
  ...
  // Reset: clear the current trip's progress, then open the trip-setup picker
  // so the user chooses which saved groups to use for the new trip.
  // Saved groups/items/names are NOT touched — those only change inside Gestione.
  const doReset = () => {
    setCheckedIds([]);
    setQtyState({});
    setExcluded([]);
  ...
  // Called when the user confirms group selection + tag in TripSetupModal
  const confirmTripSetup = ({ selected, tag }) => {
    const newHidden = allSavedCategories.filter(c => !selected.includes(c));
  ...
  const handleGestioneSave = ({ catNames: newNames, removedIds: newRemoved, expiry: newExpiry, qtys, itemNames, newGroup, deletedGroups: delGroups, newItems, expiryGroups: newExpiryGroups }) => {
    ...
    // Apply renames the user made via long-press inside Gestione
    if (itemNames && Object.keys(itemNames).length > 0) {
      setCustomItems(prev => {
        const updated = [...prev];
        for (const [id, nome] of Object.entries(itemNames)) {
          if (!nome || !nome.trim()) continue;
          const existing = updated.findIndex(x => x.id === id);
          if (existing >= 0) { updated[existing] = { ...updated[existing], nome }; }
          else {
            for (const items of Object.values(DEFAULT_DATA)) {
              const found = items.find(i => i.id === id);
              if (found) { updated.push({ ...found, nome }); break; }
            }
          }
        }
        return updated;
    ...
    // Add the real items the user created via the ➕ Aggiungi elemento modal inside Gestione
    ...
    // Track groups created with no items yet so they don't disappear,
    // and drop any group from that list once it actually has items.
    setEmptyGroups(prev => {
      let next = [...prev];
      if (newGroup && !next.includes(newGroup)) next.push(newGroup);
      const gotItems = new Set((newItems||[]).map(i => i.cat));
      next = next.filter(g => !gotItems.has(g));
      if (delGroups && delGroups.length > 0) next = next.filter(g => !delGroups.includes(g));
      return next;
    });
    ...
    if (newExpiryGroups) {
      setExpiryGroups(newExpiryGroups);
      try { localStorage.setItem(K_EXPIRYGROUPS, JSON.stringify(newExpiryGroups)); } catch {}
    }
  ...
  if (view === "gestione") {
    return <GestioneLista
      ...
      expiry={expiry}
      hiddenGroups={hiddenGroups}
      onUnhide={(cat) => setHiddenGroups(p => p.filter(x => x !== cat))}
      expiryGroups={expiryGroups}
      tripTag={tripTag}
      onTripTagChange={setTripTag}
  ...
  // ── List view ───────────────────────────────────────────────────────────────
  return (
    <div style={{ fontFamily:"'Segoe UI',system-ui,sans-serif", background:"#0F172A", minHeight:"100vh", color:"#F1F5F9", paddingBottom:"calc(20px + env(safe-area-inset-bottom, 16px))" }}>
      ...
      {/* Reset — full width, on its own row */}
      <div style={{ padding:"0 16px 10px" }}>
        <button onClick={()=>setShowConfirmReset(true)} style={{
          width:"100%", background:"#1E293B", border:"1px solid #EF4444", color:"#EF4444",
                ...
                {editingCat===cat
                  ...
                  : <>
                      ...
                      <button onClick={()=>setOpenCats(p=>({...p,[cat]:!isOpen}))} style={{
                        ...
                        cursor:"pointer", padding:"12px 4px 12px 4px"
                      }}>{isOpen?"▲":"▼"}</button>
                      <button onClick={()=>setConfirmHideGroup(cat)} style={{
                        background:"none", border:"none", color:"#475569", fontSize:15,
                        cursor:"pointer", padding:"12px 14px 12px 4px"
                      }} title="Rimuovi dal viaggio">🗑</button>
                    </>
                }
              </div>
              ...
              {isOpen && (
                <div style={{ background:"#1E293B", borderRadius:"0 0 10px 10px", overflow:"hidden" }}>
                  {(catMap[cat]||[]).filter(i=>!excluded.includes(i.id)).length === 0 ? (
                    ...
                    const isMed = expiryGroups.includes(cat);
        ...
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.78)", zIndex:200,
          ...
          <div style={{ background:"#1E293B", borderRadius:16, padding:"24px 20px", width:"100%", maxWidth:320 }}>
            ...
            <div style={{ fontSize:13, color:"#94A3B8", textAlign:"center", marginBottom:24, lineHeight:1.5 }}>
              Spunte e quantità si azzerano. Poi scegli quali liste salvate usare per questo viaggio.<br/>
              <b style={{color:"#10B981"}}>I tuoi gruppi ed elementi salvati restano intatti.</b>
            </div>
            <div style={{ display:"flex", gap:10 }}>
      ...
      {confirmHideGroup && (
        <div style={{ position:"fixed", inset:0, background:"rgba(0,0,0,.78)", zIndex:200,
          ...
          <div style={{ background:"#1E293B", borderRadius:16, padding:"24px 20px", width:"100%", maxWidth:320 }}>
            ...
            <div style={{ fontSize:13, color:"#94A3B8", textAlign:"center", marginBottom:24, lineHeight:1.5 }}>
              «{catNames[confirmHideGroup] || confirmHideGroup}» verrà rimossa da questo viaggio.<br/>
              <b style={{color:"#10B981"}}>Resta salvata in ⚙️ Gestione, puoi riattivarla quando vuoi.</b>
            </div>
            <div style={{ display:"flex", gap:10 }}>
              <button onClick={()=>setConfirmHideGroup(null)} style={{
                flex:1, background:"#334155", border:"none", borderRadius:12,
                padding:"13px 0", color:"#94A3B8", fontWeight:700, fontSize:15, cursor:"pointer"
              }}>Annulla</button>
              <button onClick={()=>{
                setHiddenGroups(p => [...p, confirmHideGroup]);
                setConfirmHideGroup(null);
              }} style={{
                flex:1, background:"#EF4444", border:"none", borderRadius:12,
                padding:"13px 0", color:"#fff", fontWeight:800, fontSize:15, cursor:"pointer"
              }}>Rimuovi</button>
